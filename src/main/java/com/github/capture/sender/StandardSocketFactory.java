 ...
 * @author yusheng
 * @version 1.0.0
 * @datetime 2021-09-07 10:21
 * @description Specialized SocketFactory to create sockets with a SOCKS proxy
 */
public class StandardSocketFactory extends SocketFactory {
    ...
    public Socket createSocket(String host, int port,
                               InetAddress localHostAddr, int localPort) throws IOException, UnknownHostException {
        Socket socket = createSocket();
        socket.bind(new InetSocketAddress(localHostAddr, localPort));
        socket.connect(new InetSocketAddress(host, port));
        return socket;
